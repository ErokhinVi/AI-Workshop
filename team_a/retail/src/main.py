...
async def api_credit_apply(payload: dict) -> dict:
    """Оркестрация заявки на кредит: решение берём в cib, сохраняем в backend.

    Сохранение в backend — best-effort: ручка хранилища кредитных заявок на
    стороне backend появится позже (часть владельца блока backend). Пока её
    нет, заявка просто не сохраняется, но клиент всё равно получает настоящее
    решение банка. Контракт сохранения (POST /credit/applications) —
    предварительный, уточняется с владельцем backend офлайн.
    """
    # 1. Решение по кредиту — блок cib (может занять несколько секунд: LLM).
    try:
    ...
    # 2. Сохранение заявки — блок backend (best-effort, ошибки не валят поток).
    application = {
        **payload,
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            s = await client.post(f"{BACKEND_URL}/credit/applications", json=application)
        saved = s.status_code == 200
    except httpx.HTTPError:
        saved = False

    return {**decision, "saved": saved}
