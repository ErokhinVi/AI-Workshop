...
async def assess_feature_liveness(client, snap: dict,
    ...
    # На Tier-1 замыкаемся ТОЛЬКО на ответах, не зависящих от валидности тела:
    #   • 2xx (успех или тело-заглушка) → работает / витрина;
    #   • 5xx → выкачено и падает.
    # 404 на «голый» вызов с пустым телом НЕ доказывает «ручки нет»: строгий банк
    # отвечает 404 «ресурс не найден» (клиента из пустого тела нет), хотя маршрут
    # существует. Не штрафуем за это — уточняем реальным вызовом с валидным телом
    # (Tier-2). Настоящий отсутствующий маршрут так и останется 404 после Tier-2 и
    # честно даст False; рабочая, но строгая ручка ответит 2xx → True.
    decisive = live is True or (live is False and status != 404)
    if decisive:
        # Однозначный ответ сервиса: 2xx-успех → работает; 5xx или 2xx с
        # телом-заглушкой → мертва.
        return _result(endpoints, primary, status=status, tier=1,
                       feature_live=live, body=body)

    # Tier 1 неубедителен (4xx-валидация или 404 «ресурс не найден»: ручка есть, но
    # нужен валидный ввод) — подтверждаем реальным вызовом с валидным телом (LLM).
