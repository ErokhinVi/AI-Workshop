...
CEIL = float(os.environ.get("CEIL", "1000.0"))  # верхняя граница базы (страховка
                                                 # от дрейфа формулы); env-настраи-
                                                 # ваема — на проде поднята/снята,
                                                 # чтобы рост клиентов был безлимитным
# Верхняя граница сводного балла для табло: 7 осей по 0..2 плюс convenience
# bonus до 6, см. main._rubric_of().
RUBRIC_MAX = 20
