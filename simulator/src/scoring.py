...
# Потолок снят по умолчанию (решение оунера): база растёт без верхнего зажима.
# Рычаг страховки от дрейфа формулы остаётся — задай CLIENT_CEIL в env, чтобы
# вернуть конечный потолок (например, CLIENT_CEIL=1000).
CEIL = float(os.environ.get("CLIENT_CEIL", "inf"))
