...
def generic_fallback(snap: dict, baseline_snap: dict | None = None) -> dict:
    ...
    if n == 0:
        axes = (0, 0, 0)
        cross_block = 0
        reason = "Автооценка: с прошлой нулевой точки контракты не изменились."
    else:
        # есть изменения, но без LLM не уверены в завершённости — ставим скромно
        axes = (1, 1, 1)
        cross_block = min(2, n)
        reason = (f"Автооценка: изменения в контрактах блоков "
                  f"({', '.join(changed)}) — добавлена новая функциональность.")
...
_JUDGE_SYSTEM = (
    ...
    "не сравнивая ни с кем. Верни СТРОГО JSON без пояснений."
)
