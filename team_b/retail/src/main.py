...
async def credit_apply(payload: dict) -> dict:
    # Решение берём напрямую в cib — синхронно, с готовым человеческим
    # объяснением. Backend как асинхронное хранилище заявок здесь не на
    # критическом пути: клиент получает решение сразу, даже если backend лежит.
    cib_payload = {
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            r = await client.post(f"{CIB_URL}/credit/decide", json=cib_payload)
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"cib недоступен: {exc}")
    if r.status_code != 200:
        raise HTTPException(status_code=r.status_code, detail=r.text[:300])
...
async def invest_recommend(payload: dict) -> dict:
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            r = await client.post(f"{CIB_URL}/invest/recommend", json=payload)
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"cib недоступен: {exc}")
    if r.status_code != 200:
        raise HTTPException(status_code=r.status_code, detail=r.text[:300])
