...
Стартовая точка для воркшопа правления. Этот блок — собиратель.
На дне старта здесь:
 - ручка `/dashboard` — собирает агрегированные цифры со всех соседей
   (через NEIGHBOR_*-переменные) и отдаёт одной JSON-страницей,
 - простая HTML-обёртка `/` чтобы можно было показывать на проекторе.

Что блок будет делать дальше (стратегия, координация, общая картина)
— решает Сергей вместе со своим AI-помощником.
"""
...
async def dashboard() -> dict:
    """Bank state — агрегаты с соседей.

    Старается мягко: если сосед не ответил — поле в ответе будет
    None, а в `errors` появится отметка какой источник упал. Так
    дашборд не падает целиком, и видно где разрыв.
    """
    started = time.time()
