...
3. Судья: LLM по каждой команде отдельно, `temperature=0`, без сравнения с
   ...
   Шум: один вызов на том же банке дает то 13, то 15 из 20, это 20-40
   клиентов. Поэтому банк судится `JUDGE_SAMPLES` вызовами (3), по каждой оси
   медиана. Одинаковый промпт судится один раз и запоминается: на старте все
   команды равны шаблону и получают один балл, а коммит, который не поменял
   того, что видит судья, базу не двигает.
...
Установщик собирает `tools/setup/make-bootstrap.py`, один на все команды.
Участник выбирает команду, блок и пишет имя. Ролей по именам нет: блок
выбирает сам участник, сменить блок значит запустить установщик еще раз.
Скрипт:

1. кладет deploy key выбранной команды в `~/.ssh/raif_workshop`;
...
репозитория, а ключ аккаунта в скрипт класть нельзя. В установщике ключи
всех команд, на ноутбук ложится один. Готовые скрипты лежат вне
репозитория, `tools/setup/check_no_keys.py` в CI не пустит ключ в
репозиторий.
